...
int qtde_Rotacoes(avl *arv) { return arv->rotacoes; }
...
no *encontraNo(avl *arv, int valor) {
    no *atual = arv->sentinela->Fdir; // Começa a busca pela raiz da árvore

    while (atual != NULL) {
        if (valor == atual->chave) {
            return atual; // Elemento encontrado
        } else if (valor < atual->chave) {
            atual = atual->Fesq; // Busca na subárvore esquerda
        } else {
            atual = atual->Fdir; // Busca na subárvore direita
        }
    }
